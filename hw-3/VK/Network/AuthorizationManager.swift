...
extension VKLoginFormConroller: WKNavigationDelegate {
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationResponse: WKNavigationResponse, decisionHandler: @escaping (WKNavigationResponsePolicy) -> Void) {
        ...
        Alamofire.request(urlFriendsList, method: .get, parameters: parameters).responseData { repsonse in
            guard let data = repsonse.value else { return }
            //let json = JSON(data: data)
            let vkontakte = try! JSONDecoder().decode(UsersResponse.self, from: data).itemsUsers
                      print(vkontakte)
                  
        
          // let vkontakte = try JSONDecoder().decode(UsersResponse.self, from: data).items
          // print(vkontakte)
        
        
          //Получение фотографий
                let urlPhotos = "https://api.vk.com/method/photos.get"
                let idUsersForPhotos: String = "229776126"
                             let parameters: Parameters = [
                                 "owner_id": idUsersForPhotos,
                                 "album_id": "profile",
                                 "photo_ids": "",
                                 "rev": "0",
                                 "extended": "0",
